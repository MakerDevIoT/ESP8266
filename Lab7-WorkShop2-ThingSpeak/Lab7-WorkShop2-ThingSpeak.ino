...
const char* ssid     = "Jirawin"; 
const char* password = "Win460220";
...
void loop() {
    ...
    if(h_dht < 30 || t_dht > 30) // condition for make relay on ถ้าความชื้นมากกว่า 30
    {
      digitalWrite(PUMP_RLY, LOW); //If condition true do this!
      tone(buzzer, 1000); // Send 1KHz sound signal... ความถี่ของเสียงเตือน
...
void uploadThingsSpeak(float t, float h) {
    static const char* host = "api.thingspeak.com"; ///////////////// ไม่ต้องแก้ไข
    static const char* apiKey = "BAOU7MFEEADH4J6Z"; /////////////////////// ให้ใช้ Write API Key ของตัวเองที่สมัครไว้
    ...
    // We now create a URI for the request ต้องเข้าใจและรู้ว่าแต่ละฟิลด์ใช้เก็บอะไรกี่ฟิลด์ ต้องมีจำนวนเท่ากัน
    String url = "/update/";
    //  url += streamId;
    url += "?key=";
    url += apiKey;
