...
void drawOilprice() {
  ...
  tmpNow += String(newtime->tm_hour);  // 12  01
  if (dots) tmpNow += ":"; else tmpNow += " ";  // : 
  if ((newtime->tm_min) < 10) tmpNow += "0";
  tmpNow += String(newtime->tm_min);  // 12  01
  tmpNow += "     ";
  ...
  tmpNow += tmpYear.substring(2, 6);  //2561  = 61
...
void updateAnto() {
  HTTPClient http;
  http.begin("http://api.anto.io/KaxSPEgtrh461961br6541ergg46gerug8VW/gas.95e10.price");
  int httpCode = http.GET();
  if (httpCode > 0) {
    String payload = http.getString();
    payload = payload.substring(26, 31);  //{"result":"true","value":"28.05"} 26, 31 = 28.05
    gas95 = payload.toFloat(); //28.05
    gas95 = gas95 + LocalCharge;  // 28.05+2.36 = 30.41
    int price3 = (int)(gas95);  //  30
    price2 = String(gas95);  // 30.41
    price1 = String(price3);  // 30
    price2 = price2.substring(2, 5);  //  .41
